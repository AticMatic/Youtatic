...
# In a real app, you'd have a proper form and validation
...
def register_api_user():
    ...
    try:
        ...
        return jsonify({'success': True, 'email': email, 'api_key': new_user.api_key})
    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Failed to register user {email}: {e}")
